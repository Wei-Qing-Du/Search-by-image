...
#        w_fc1 = tf.Variable(weight_variable([dim,256],0.005), name='weights',dtype=tf.float32)
        w_fc1 = tf.Variable(tf.truncated_normal(shape=[dim,384],stddev=0.005,dtype=tf.float32),
                            name='weights',dtype=tf.float32)#Init weights
#        b_fc1 = tf.Variable(bias_variable([256]),name='biases',dtype=tf.float32)
        b_fc1 = tf.Variable(tf.constant(value=0.1,dtype=tf.float32,shape=[384]),
...
#        w_fc2 = tf.Variable(weight_variable([256,256],0.005),name='weights',dtype=tf.float32)
        w_fc2 = tf.Variable(tf.truncated_normal(shape=[384,192],stddev=0.005,dtype=tf.float32),
                            name='weights',dtype=tf.float32)
#        b_fc2 = tf.Variable(bias_variable([256]),name='biases',dtype=scope.name)
        b_fc2 = tf.Variable(tf.constant(value=0.1,dtype=tf.float32,shape=[192]),
    ...
    with tf.variable_scope('softmax_liner') as scope:
        weights = tf.Variable(tf.truncated_normal(shape=[192,n_classes],stddev=0.005,dtype=tf.float32)
                              ,name='softmax_liner',dtype=tf.float32)
        biases = tf.Variable(tf.constant(value=0.0,dtype=tf.float32,shape=[n_classes])
                             ,name='biases',dtype=tf.float32)
